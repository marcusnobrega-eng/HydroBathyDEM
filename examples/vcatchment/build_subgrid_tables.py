...
MESH_NC = HERE / "vcatchment_mesh.nc"
OVERLAP_NC = HERE / "vcatchment_mesh_overlap.nc"
DATUM_MODE = sys.argv[1] if len(sys.argv) > 1 else "bed_plane"
OUT_NC = HERE / (f"vcatchment_subgrid_{DATUM_MODE}.nc")
...
def main() -> None:
    ...
    # The face is a sill at the controlling invert: the HIGHER of the two adjacent
    # cell datums.  Without this the face reads wet on dry ground (see the note in
    # build_face_conveyance_table) and the hydrograph is destroyed.
    control = np.where(
        edge_neighbor >= 0,
        np.maximum(cells.datum_m[edge_owner], cells.datum_m[np.maximum(edge_neighbor, 0)]),
        cells.datum_m[edge_owner],
    )
    faces = build_face_conveyance_table(profiles, edge_length, control_elevation_m=control)
