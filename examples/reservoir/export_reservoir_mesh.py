"""Write the reservoir mesh as UGRID + conservative overlap."""
...
OUT = Path("examples/reservoir")
cells = pickle.load(open(OUT/"_cells.pkl","rb"))
geo = json.load(open(OUT/"reservoir_geometry.json"))
with rasterio.open(OUT/"reservoir_dem_5m.tif") as src:
    dem = src.read(1); transform, crs = src.transform, src.crs
with rasterio.open(OUT/"reservoir_manning_5m.tif") as src:
    nras = src.read(1)
...
with netCDF4.Dataset(nc, "a") as ds:
    ...
    ds.subgrid_dem_path = str((OUT/"reservoir_dem_5m.tif").resolve())
    ds.subgrid_manning_path = str((OUT/"reservoir_manning_5m.tif").resolve())
ov = write_conservative_overlap(nc, gpkg, OUT/"reservoir_dem_5m.tif", OUT/"reservoir_mesh_overlap.nc")
