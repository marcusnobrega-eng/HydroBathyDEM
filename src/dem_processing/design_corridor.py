...
it solves compound Manning conveyance at each D4 river cell using the mapped
...
from .design_hydrograph import receiver_from_d4_direction
...
class DesignCorridorConfig:
    ...
    def from_mapping(cls, values: dict[str, Any]) -> "DesignCorridorConfig":
        values = dict(values)
        grouped = {name: values.pop(name, {}) for name in ("inputs", "hydraulics")}
        for name, group in grouped.items():
            if group and not isinstance(group, dict):
                raise ValueError(f"{name} must be a configuration object.")
        values = {**grouped["inputs"], **grouped["hydraulics"], **values}
        aliases = {
...
def _fill_terminal_river_directions(
    direction: np.ndarray, river: np.ndarray, width: np.ndarray,
    ...
    receiver = receiver_from_d4_direction(result)
...
def _cross_section_indices(row: int, col: int, direction: int, first_offset: int, reach_cells: int, shape: tuple[int, int]) -> tuple[np.ndarray, np.ndarray]:
    """Return near-bank-to-outside cells at the left and right of a D4 link."""
    ...
    normal = {1: (0, -1), 2: (-1, 0), 3: (0, 1), 4: (1, 0)}.get(direction)
    if normal is None:
...
def _river_reaches(receiver: np.ndarray, active: np.ndarray) -> list[list[int]]:
    """Directed, junction-to-junction reaches for an active D4 river graph."""
...
def _sample_positions(path: list[int], shape: tuple[int, int], cell_width: float, spacing_m: float) -> tuple[np.ndarray, np.ndarray]:
    ...
    for index, (left, right) in enumerate(zip(path[:-1], path[1:], strict=True), start=1):
        ...
        distance[index] = distance[index - 1] + (cell_width if lc != rc else cell_width)
...
def build_design_corridor(config: DesignCorridorConfig) -> dict[str, Any]:
    with rasterio.open(config.dem) as source:
        ...
        direction = _aligned(config.river_direction, source, "D4 flow direction")
    ...
    direction, direction_fallback_cells = _fill_terminal_river_directions(direction, river, width)
    ...
    receiver = receiver_from_d4_direction(direction)
    cell_width = float(abs(profile["transform"].a))
    reach_cells = max(1, int(np.ceil(config.max_half_width_m / cell_width)))
    ...
    for path in _river_reaches(receiver, river):
        distance, samples = _sample_positions(path, terrain.shape, cell_width, config.cross_section_spacing_m)
        sample_stage = np.full(len(samples), np.nan)
        sample_bank = np.full(len(samples), np.nan)
        sample_depth = np.full(len(samples), np.nan)
        bank_cells = max(1, int(np.ceil(config.bank_sampling_width_m / cell_width)))
        for sample_index, path_index in enumerate(samples):
            row, col = divmod(path[path_index], terrain.shape[1])
            first_offset = max(1, int(np.ceil(0.5 * width[row, col] / cell_width)))
            left, right = _cross_section_indices(row, col, int(direction[row, col]), first_offset, reach_cells, terrain.shape)
        ...
        for sample_index, path_index in enumerate(samples):
            ...
            water_level, solved = _solve_stage(
                ...
                left_ground, right_ground, cell_width, config.channel_manning_n, config.floodplain_manning_n, config.max_stage_above_bank_m,
            )
            if solved:
        ...
        for path_index, node in enumerate(path):
            ...
            left_half_width[row, col] = wet_left.sum() * cell_width + 0.5 * width[row, col]
            right_half_width[row, col] = wet_right.sum() * cell_width + 0.5 * width[row, col]
    ...
    # floodplain cell reaches the same D4 river network used by the hydraulic
    ...
    outputs = {
        "D4_mesh_refinement_Qp_envelope_100yr_m3s.tif": refinement_peak,
        "D4_design_water_level_100yr_m.tif": stage,
        "D4_design_overbank_half_width_100yr_m.tif": half_width,
        "D4_design_left_half_width_100yr_m.tif": left_half_width,
        "D4_design_right_half_width_100yr_m.tif": right_half_width,
        "D4_design_lateral_search_cap.tif": lateral_search_cap,
        "D4_design_floodplain_mask_100yr.tif": floodplain.astype(np.uint8),
        "D4_design_corridor_status.tif": status,
        "D4_design_floodplain_hydraulic_potential_m.tif": hydraulic_potential,
        "D4_design_floodplain_flow_direction.tif": floodplain_direction,
        "D4_design_floodplain_flow_accumulation_km2.tif": floodplain_accumulation * cell_area_km2,
        "D4_design_floodplain_axis_mask.tif": floodplain_axis.astype(np.uint8),
    }
    ...
    for name, data in outputs.items():
        ...
        with rasterio.open(path, "w", **out_profile) as target:
            target.write(data, 1)
        paths[name] = str(path)
    vector_path = config.out_dir / "D4_design_floodplain_corridor_100yr.gpkg"
